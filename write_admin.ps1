    ...
    if (estado === 'convertido' && !prospect.user_id) {
      try {
        ...
        if (userId) {
          ...
          toast.dismiss();
          toast.success(`Usuario creado: ${prospect.email} pendiente de aprobacion`);
          return;
        }
      } catch (err) {
        toast.dismiss();
...
'@
$content | Out-File -FilePath "apps\web\src\pages\AdminPage.jsx" -Encoding UTF8
Write-Host "Done: $((Get-Content apps\web\src\pages\AdminPage.jsx).Length) lines"
