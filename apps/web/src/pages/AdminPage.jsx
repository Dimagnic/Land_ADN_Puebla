...
function CMSBenefits() {
  ...
  const updateItem = (idx, field, val) =>
    setData(p => ({ ...p, items: p.items.map((it, i) => i === idx ? { ...it, [field]: val } : it) }));
  const addItem = () =>
    setData(p => ({ ...p, items: [...p.items, { icon: 'BookOpen', title: '', desc: '' }] }));
          ...
          <div key={idx} className="border rounded-lg p-3 bg-muted/30">
            <div className="grid grid-cols-2 gap-2 mb-2">
              <div>
                ...
                <input value={item.icon} onChange={e => updateItem(idx, 'icon', e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className="text-xs text-muted-foreground">Título</label>
                <input value={item.title} onChange={e => updateItem(idx, 'title', e.target.value)} className={inputCls} />
              </div>
            </div>
            <label className="text-xs text-muted-foreground">Descripción</label>
            <input value={item.desc} onChange={e => updateItem(idx, 'desc', e.target.value)} className={inputCls + ' mb-2'} />
            <Button variant="ghost" size="sm" onClick={() => removeItem(idx)} className="text-destructive h-7 px-2">
...
function SocialsAdmin() {
  ...
  const [saving, setSaving] = useState(false);
  useEffect(() => {
    supabase.from('socials').select('*').order('sort_order').then(({ data }) => data && setItems(data));
  }, []);
  const update = (id, field, val) => setItems(p => p.map(s => s.id === id ? { ...s, [field]: val } : s));
  const save = async () => {
    setSaving(true);
    try {
      await Promise.all(items.map(s => supabase.from('socials').update({ url: s.url }).eq('id', s.id)));
        ...
        <div className="rounded-lg border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted">
              <tr>
                {['Nombre', 'Código', 'Patrocinador', 'Rol', 'Registro', 'Acción'].map(h => (
            ...
            <tbody>
              {filtered.map(u => (
                <tr key={u.id} className="border-t hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{u.nombre_patrocinador || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${ROLE_COLOR[u.role] || ''}`}>{u.role}</span>
                  </td>
      ...
      {loading ? <Spinner /> : (
        <div className="rounded-lg border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted">
              <tr>
                {['Nombre', 'Email', 'Teléfono', 'Interés', 'Estado', 'Fecha', 'Acción'].map(h => (
                  <th key={h} className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className="border-t hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs font-semibold ${STATE_COLOR[p.estado] || ''}`}>{p.estado}</span></td>
...
function TestimoniosAdmin() {
  ...
  const load = () => {
    supabase.from('testimonios').select('*').order('fecha_creacion', { ascending: false })
      .then(({ data }) => { setItems(data || []); setLoading(false); });
  };
  useEffect(() => { load(); }, []);
  ...
  const save = async (item) => {
    ...
    try {
      if (item._new) {
        const { data } = await supabase.from('testimonios').insert([{ nombre: item.nombre, cargo: item.cargo, contenido: item.contenido, activo: item.activo }]).select().single();
        setItems(p => p.map(x => x.id === item.id ? data : x));
      } else {
        await supabase.from('testimonios').update({ nombre: item.nombre, cargo: item.cargo, contenido: item.contenido, activo: item.activo }).eq('id', item.id);
      }
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <Button size="sm" onClick={add} className="gap-1"><Plus className="w-4 h-4" />Agregar</Button>
      </div>
        ...
        <div key={item.id} className="border rounded-lg p-4 mb-3 bg-muted/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-2 text-sm ml-auto cursor-pointer">
              <input type="checkbox" checked={item.activo} onChange={e => upd(item.id, 'activo', e.target.checked)} />Activo
            </label>
          </div>
        </div>
      ))}
...
function ModulesAdmin() {
  ...
  const load = () => {
    supabase.from('modules').select('*, lessons(*)').order('order_num')
  ...
  const saveModule = async (mod) => {
    setSaving(mod.id);
    try {
      await supabase.from('modules').update({ title: mod.title, description: mod.description, active: mod.active }).eq('id', mod.id);
  ...
  const saveLesson = async (lesson) => {
    setSaving(lesson.id);
    try {
      await supabase.from('lessons').update({ title: lesson.title, description: lesson.description, video_url: lesson.video_url, content: lesson.content, active: lesson.active }).eq('id', lesson.id);
  ...
  const updLesson = (moduleId, lessonId, f, v) =>
    setModules(p => p.map(m => m.id === moduleId ? { ...m, lessons: m.lessons.map(l => l.id === lessonId ? { ...l, [f]: v } : l) } : m));
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <Button size="sm" onClick={addModule} className="gap-1"><Plus className="w-4 h-4" />Agregar módulo</Button>
      </div>
      {modules.map(mod => (
                  ...
                  <div key={lesson.id} className="border rounded-lg p-3 mb-3 bg-background">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <label className="flex items-center gap-1 text-xs ml-auto cursor-pointer">
                        <input type="checkbox" checked={lesson.active} onChange={e => updLesson(mod.id, lesson.id, 'active', e.target.checked)} />Activa
                      </label>
                    </div>
                  </div>
                ))}
        ...
        <div key={item.id} className="border rounded-lg p-4 mb-3">
          <div className="grid grid-cols-2 gap-3 mb-2">
            ...
            <div><label className="text-xs text-muted-foreground">Tipo</label>
              <select value={item.type} onChange={e => upd(item.id, 'type', e.target.value)} className={inputCls}>
...
function ResourcesAdmin() {
  ...
  const save = async (item) => {
    ...
    try {
      ...
      if (item._new) {
        const { data } = await supabase.from('resources').insert([payload]).select().single();
        setItems(p => p.map(x => x.id === item.id ? data : x));
      } else {
        await supabase.from('resources').update(payload).eq('id', item.id);
      }
      ...
      {loading ? <Spinner /> : items.map(item => (
        <div key={item.id} className="border rounded-lg p-4 mb-3">
          <div className="grid grid-cols-2 gap-3 mb-2">
            ...
            <div><label className="text-xs text-muted-foreground">Tipo</label>
              <select value={item.type} onChange={e => upd(item.id, 'type', e.target.value)} className={inputCls}>
                {['document','image','link','video'].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
...
const NAV = [
  ...
  { id: 'prospectos',   label: 'Prospectos',     icon: Users,      section: 'Educativo' },
  { id: 'usuarios',     label: 'Usuarios',       icon: Users },
  { id: 'modulos',      label: 'Módulos/Lecciones', icon: BookOpen },
  { id: 'eventos',      label: 'Eventos',        icon: Calendar },
  { id: 'recursos',     label: 'Recursos',       icon: Download },
  { id: 'anuncios',     label: 'Anuncios',       icon: Megaphone },
];

const TITLES = { dashboard:'Dashboard', 'cms-hero':'Hero', 'cms-header':'Header', 'cms-footer':'Footer', 'cms-benefits':'Beneficios', 'cms-seo':'SEO', socials:'Redes Sociales', testimonios:'Testimonios', prospectos:'Prospectos', usuarios:'Usuarios', modulos:'Módulos y Lecciones', eventos:'Eventos', recursos:'Recursos', anuncios:'Anuncios' };
      ...
      <aside className="w-56 bg-card border-r flex flex-col overflow-y-auto flex-shrink-0">
        ...
        <nav className="p-2 flex-1">
          {NAV.map(item => (
            <div key={item.id}>
              {item.section && (
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground px-2 pt-4 pb-1">{item.section}</p>
              )}
