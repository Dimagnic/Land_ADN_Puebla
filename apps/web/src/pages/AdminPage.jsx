...
function ProspectsAdmin() {
  ...
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  useEffect(() => { supabase.from('prospects').select('*').order('created_at', { ascending: false }).then(({ data }) => { setItems(data || []); setLoading(false); }); }, []);
  const updateEstado = async (id, estado) => {
    await supabase.from('prospects').update({ estado }).eq('id', id);
    setItems(p => p.map(x => x.id === id ? { ...x, estado } : x));
  };
  const filtered = items.filter(u => u.nombre.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div>
      ...
      {loading ? <Spinner /> : (
        <div className="rounded-lg border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted"><tr>{['Nombre', 'Email', 'Teléfono', 'Interés', 'Estado', 'Fecha', 'Acción'].map(h => <th key={h} className="text-left px-4 py-3 font-medium text-muted-foreground text-xs uppercase">{h}</th>)}</tr></thead>
            <tbody>
              {filtered.map(p => (
                <tr key={p.id} className="border-t hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3"><select value={p.estado} onChange={e => updateEstado(p.id, e.target.value)} className="text-xs border rounded px-2 py-1 bg-background">{ESTADOS.map(s => <option key={s} value={s}>{s}</option>)}</select></td>
                </tr>
              ))}
            </tbody>
          </table>
